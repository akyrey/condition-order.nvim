...
describe("Python analyzer", function()
	it("flags expensive call before cheap variable in 'and'", function()
		local bufnr = make_buf("python", {
			"import re",
			"def f(flag, name):",
			"  if re.match(r'pattern', name) and flag:",
			"    pass",
		})
		local n = diag_count(bufnr, "python")
		del_buf(bufnr)
		assert.is_true(n >= 1, "expected at least 1 diagnostic")
	end)

	it("does not flag correctly ordered 'and' chain", function()
		local bufnr = make_buf("python", {
			"import re",
			"def f(flag, name):",
			"  if flag and re.match(r'pattern', name):",
			"    pass",
		})
		local n = diag_count(bufnr, "python")
		del_buf(bufnr)
	...
	it("flags expensive call before cheap variable in 'or'", function()
		local bufnr = make_buf("python", {
			"import re",
			"def f(flag, name):",
			"  if re.search(r'admin', name) or flag:",
			"    pass",
		})
		local n = diag_count(bufnr, "python")
		del_buf(bufnr)
		assert.is_true(n >= 1, "expected at least 1 diagnostic for 'or' chain")
	end)

	it("respects ignore comment", function()
		local bufnr = make_buf("python", {
			"import re",
			"def f(flag, name):  # condition-order: ignore",
			"  if re.match(r'admin', name) and flag:",
			"    pass",
		})
		-- The comment is on a different line than the condition; ignore only applies
		-- when the comment is on one of the condition's lines.
		-- This test verifies no crash — exact behavior depends on line matching.
		local n = diag_count(bufnr, "python")
		del_buf(bufnr)
		assert.is_true(n >= 0)
	end)

	it("negation is transparent: 'not call()' costs same as 'call()'", function()
		local buf1 = make_buf("python", {
			"import re",
			"def f(flag, name):",
			"  if not re.match(r'admin', name) and flag:",
			"    pass",
		})
		...
		local n1 = diag_count(buf1, "python")
		local n2 = diag_count(buf2, "python")
		del_buf(buf1)
		del_buf(buf2)
		assert.are.equal(n2, n1, "negation should not change whether a diagnostic fires")
	end)

	it("blocks fix for side-effecting calls", function()
		local bufnr = make_buf("python", {
			"def f():",
			"  if open('a') and open('b'):",
			"    pass",
		})
		local diag_n = diag_count(bufnr, "python")
		local fix_n = fix_count(bufnr, "python")
		del_buf(bufnr)
		if diag_n > 0 then
			assert.are.equal(0, fix_n, "fix should be blocked for side-effecting calls")
		end

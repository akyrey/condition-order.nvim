...
--- Extract the logical operator text from a logical-binary node.
...
local function contains_impure_call(node, bufnr, lang)
	local spec = cost.get_language(lang)
	local ntype = node:type()

	local call_types = spec and spec.call_node_types or {}
	...
	if call_types[ntype] then
		...
		if fname then
			...
			if cost.is_known_pure(fname, lang) then
				return false
			end
		end
	...
	for child in node:iter_children() do
		if contains_impure_call(child, bufnr, lang) then
			return true
		end
	end
	return false
...
local function has_comment_node(node)
	...
	for child in node:iter_children() do
		if has_comment_node(child) then
			return true
		end
	end
	return false
...
local function guarded_var(text)
	...
	-- Go / Python: identifier != nil/None  /  identifier == nil/None
	v = text:match("^([%a_][%w_]*)%s*[!=][!=]?%s*nil%f[%W]")
	if v and v ~= "nil" and v ~= "true" and v ~= "false" then
		return v
	end

	v = text:match("^([%a_][%w_]*)%s*[!=][!=]?%s*[Nn]one%f[%W]")
	if v and v ~= "None" then
		return v
	end

	-- reversed: nil/None != identifier
	v = text:match("^nil%s*[!=][!=]?%s*([%a_][%w_]*)%f[%W]")
	if v and v ~= "nil" then
		return v
	end

	v = text:match("^[Nn]one%s*[!=][!=]?%s*([%a_][%w_]*)%f[%W]")
	if v and v ~= "None" then
		return v
	end

	-- Python: identifier is not None  /  identifier is None
	v = text:match("^([%a_][%w_]*)%s+is%s+not%s+None%f[%W]")
	if v then
		return v
	end

	v = text:match("^([%a_][%w_]*)%s+is%s+None%f[%W]")
	if v then
		return v
	end
...
local function accesses_var_as_base(op_text, var_name)
	...
	return t:match("^" .. e .. "%->") ~= nil -- PHP property/method
		or t:match("^" .. e .. "%[") ~= nil -- PHP/Go/Python index
		or t:match("^" .. e .. "%.") ~= nil -- Go/Python field/method
end
...
local function flatten_chain(node, operator, bufnr, lang, spec, operands)
	local binary_types = spec.logical_binary_node_types or { binary_expression = true }

	if not binary_types[node:type()] then
		-- Leaf operand
	...
	if left then
		flatten_chain(left, operator, bufnr, lang, spec, operands)
	end
	if right then
		flatten_chain(right, operator, bufnr, lang, spec, operands)
	end
end
...
--- Analyse a single logical-binary node that is the top of a logical chain.
---@param node        TSNode
---@param bufnr       integer
---@param lang        string
---@param spec        table  language spec
...
local function analyze_binary(node, bufnr, lang, spec, diagnostics, fixes)
	...
	local bin_types = spec.logical_binary_node_types or { binary_expression = true }
	if parent and bin_types[parent:type()] then
		if get_logical_op(parent, bufnr) == operator then
			return
		end
	end
	...
	if ignore and ignore ~= "" then
		...
		for _, line in ipairs(lines) do
			if line:find(ignore, 1, true) then
				return
			end
		end
	end
	...
	-- Flatten the chain.
	local operands = {}
	flatten_chain(node, operator, bufnr, lang, spec, operands)
	if #operands < 2 then
		return
	end
	...
	table.sort(sorted, function(a, b)
		if a.op.cost ~= b.op.cost then
			return a.op.cost < b.op.cost
		end
		return a.idx < b.idx -- preserve original order for ties
	end)
	local sorted_ops = {}
	for _, s in ipairs(sorted) do
		sorted_ops[#sorted_ops + 1] = s.op
	end
	...
	-- For ||/or: note that "cheapest first" is a fallback heuristic; ideal is
	...
	local msg
	if fix_blocked then
		msg = string.format("Conditions out of order for faster %s — manual fix needed (%s)", op_hint, fix_reason)
	else
...
local function walk(node, bufnr, lang, spec, diagnostics, fixes, in_condition)
	local ntype = node:type()
	local next_cond = in_condition

	if spec.func_boundaries and spec.func_boundaries[ntype] then
		next_cond = false
	elseif spec.condition_starters and spec.condition_starters[ntype] then
		next_cond = true
	elseif spec.body_nodes and spec.body_nodes[ntype] then
		next_cond = false
	end

	local bin_types = spec.logical_binary_node_types or { binary_expression = true }
	if bin_types[ntype] and in_condition then
		analyze_binary(node, bufnr, lang, spec, diagnostics, fixes)
	end

	for child in node:iter_children() do
		walk(child, bufnr, lang, spec, diagnostics, fixes, next_cond)
	end
end
...
local function run_analysis(bufnr, lang)
	...
	parser:for_each_tree(function(tree, lang_tree)
		...
		if not vim.tbl_contains(config.options.filetypes, effective) then
			return
		end

		local spec = cost.get_language(effective)
		if not spec then
			return
		end

		local root = tree:root()
		walk(root, bufnr, effective, spec, diagnostics, fixes, false)
	end)
